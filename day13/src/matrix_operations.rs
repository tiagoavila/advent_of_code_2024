mod matrix_operations {
    pub fn gaussian_elimination(matrix: &mut Vec<Vec<f64>>) -> Option<(usize, usize)> {
        ...
        let a = matrix[0][2] as usize;
        let b = matrix[1][2] as usize;
...
mod tests {
    ...
    fn test_gaussian_elimination_on_example_1() {
        let mut matrix = vec![
            vec![94.0, 22.0, 8400.0],
            vec![34.0, 67.0, 5400.0],
        ];
        let result: Option<(usize, usize)> = Some((80, 40));
        assert_eq!(matrix_operations::gaussian_elimination(&mut matrix), result);
    }

    #[test]
    fn test_gaussian_elimination_on_example_2() {
        let mut matrix = vec![
            vec![26.0, 67.0, 12748.0],
            vec![66.0, 21.0, 12176.0],
        ];
    ...
    fn test_gaussian_elimination_on_example_3() {
        let mut matrix = vec![
            vec![17.0, 84.0, 7870.0],
            vec![86.0, 37.0, 6450.0],
        ];
    ...
    fn test_gaussian_elimination_on_example_4() {
        let mut matrix = vec![
            vec![69.0, 27.0, 18641.0],
            vec![23.0, 71.0, 10279.0],
        ];
        let result: Option<(usize, usize)> = None;
        assert_eq!(matrix_operations::gaussian_elimination(&mut matrix), result);
    }
